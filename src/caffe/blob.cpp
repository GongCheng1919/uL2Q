...
void Blob<Dtype>:: ulq_weights(Phase phase,CompressParameter compress_param){
	...
	//alpha save scale，delta save shift，maxbits save quantization bit-width，fixedpos is not used.
	if(maxbits_<=0){return;}
	if (!data_) { return; }
	CHECK(data_);
	switch (data_->head()) {
		case SyncedMemory::HEAD_AT_CPU:
		{	//calc alpha_ and delta_(mean and std)
			...
			//Dtype downs=1-ups;
			//using 3.x sigma instead of max/min values
			if(this->maxbits_>8){
...
void Blob<double>::ToProto(BlobProto* proto, bool write_diff) const {
  ...
  //GC:save ternary or quantize values.
  ...
  if(save_ternary&&delta_!=0&&alpha_!=0){
	...
	if(fixedpos_!=0){
		...
		proto->set_alpha(get_alpha()/scaler);
		proto->set_fixedpos(get_fixedpos());
		proto->set_maxbits(get_maxbits());
	}else{
		proto->set_alpha(get_alpha());
